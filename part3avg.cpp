...
//
void print_stats(ostream &stats_file,int tot_arrivals ,double avg_service_time)
{
	stats_file <<tot_arrivals <<" "<<avg_service_time<< ",  "<<endl;
}
...
int main() {
	...
	stats_file.open ("part3Q3.txt"); //it creates the file, if it does not exist
	
	for(long int tot_arrivals=10000;tot_arrivals<=100000;)
	{
	...
	print_stats(stats_file, tot_arrivals,avg_service_time);
	tot_arrivals+=10;
	}
	stats_file.close();
	return 0;
